from widthselector import width_selector_M1, width_selector_M2
import subprocess
import time
import os
...
def modifyScript(script, data_file):
    with open(script, 'r') as f:
        ...
        for i, line in enumerate(allLines):
            if 'cat' in line:
                line = line[:line.find('$')]+data_location+data_file+line[line.find('|')-1:]
                allLines[i] = line
...
for data in data_files:
    ...
    #subprocess.run(["./evaluate-methods.sh", datapath, script_location], stdout=subprocess.DEVNULL, stderr=log)
    log.close()
