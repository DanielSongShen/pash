...
for data in data_files:
    ...
    with open("overhead/M1_times"+data_name, 'w') as f:
        for i in range(len(script_list)):
            #f.write(script_names[i]+'\n')
            f.write(str(M1_times[i])+'\n')

    with open("overhead/M2_times"+data_name, 'w') as f:
        for i in range(len(script_list)):
            #f.write(script_names[i]+'\n')
            f.write(str(M2_times[i])+'\n')

    with open("overhead/M1_widths"+data_name, 'w') as f:
        for i in range(len(script_list)):
            #f.write(script_names[i]+'\n')
            f.write(str(M1_widths[i])+'\n')

    with open("overhead/M2_widths"+data_name, 'w') as f:
        for i in range(len(script_list)):
            #f.write(script_names[i]+'\n')
